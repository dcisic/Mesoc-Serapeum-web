...
<?php
$sql1 ="select  CityName from city where ID_City= ". $CID;
$resultsx= mysqli_query($db,$sql1);
foreach ( $resultsx as $resultx){
    $CITY=$resultx['CityName'];
     }
$sql="SELECT Id_Doc, Author,Title,Summary Abstract,Links FROM `document` where Id_Doc in ( select ID_Doc from doccity where ID_City =".$CID.")";
...
?>
 ...
 <h1 class="counter_value mt-12" data-target="370">Document(s) regarding <?php echo $CITY ;?></h1>
                        <p class="mb-0 mt-2 text-uppercase">Documents that are regarding certain city, region or country </p>
                    </div>
                </div>
	        </div>
        </div>
